...
class TextPrint:
    ...
    def print(self, screen, textString):
        textBitmap = self.font.render(textString, True, FONTGREEN)
...
while done==False:
    ...
    value = (value * -1) + 100
    value = str(value)
    if value != EnginePwrState:
        Edata = 'Engine-' + value
        Edata = Edata.ljust(32)
        communicate(value)
        EnginePwrState = value
    Evalue = "Engine Power: " + value
    textPrint.print(screen, Evalue)
    TreverseButton = joystick.get_button (0)

    TreversePrint = "Reverse Throttle: " + str(TreverseState)
    textPrint.print(screen, TreversePrint)
    if TreverseButton != TreverseState:
        TreverseState = TreverseButton
        TreverseMsg = "Reverse-Toggle"
        TreverseMsg = TreverseMsg.ljust(32)
        client.send(TreverseMsg.encode())
